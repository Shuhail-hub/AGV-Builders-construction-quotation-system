...
interface SupplierLoginProps {
  onLogin: (supplier: any) => void;
}

export default function SupplierLogin({ onLogin }: SupplierLoginProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Mock authentication
    if (username === 'lanka_supplies' && password === 'supplier123') {
      const supplierData = {
        name: 'Lanka Building Supplies',
        type: 'Cement & Steel',
        username: 'lanka_supplies'
      };
    ...
    >
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <p className="text-gray-600 text-sm">Access your orders dashboard</p>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <p className="text-center text-sm text-gray-500">
              Demo: lanka_supplies / supplier123
            </p>
          </form>
        </CardContent>
      </Card>
